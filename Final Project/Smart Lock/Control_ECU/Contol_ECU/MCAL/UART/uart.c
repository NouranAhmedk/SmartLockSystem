...
void UART_init(const UART_ConfigType *Config_Ptr) {
	...
	/**************In Case Using INTERRUPT In Receiving** ******************
	 UCSRB = (1 << RXEN) | (1 << TXEN) | (1 << RXCIE);
	 UCSRB = (1<<RXEN) | (1<<TXEN) | ((Config_Ptr->reciver_interrupt) << RXCIE); if i added it
	 */
...
/*********** Another Method For Receiving bit by interrupt *********
 ISR(USART_RXC_vect) {
 receivedData = UDR;
 Handle receivedData as needed
 }
 *******************************************************************/
